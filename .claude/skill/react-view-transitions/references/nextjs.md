...
List items wrapped in ViewTransition with key will animate reorder. This is the server-component alternative to the client-side `useDeferredValue` pattern in `patterns.md`.
...
Next.js loading.tsx is an implicit Suspense boundary. Wrap the skeleton in ViewTransition with exit prop in loading.tsx, and the content in ViewTransition with enter prop and default="none" in the page:
...
- Link with transitionTypes works in Server Components — no 'use client' needed
