# Day 11 — The JS snippet

## Context
Days 1–10 done. Today we build the JS snippet — the most important piece of the product. This version is a complete, GDPR-ready implementation with:
- **Granular consent categories** (necessary / analytics / marketing) with a "Customize" panel
- **Consent invalidation** — if the site owner saves new config, `configVersion` changes and the banner re-appears automatically
- **Client-side cookie scanning** — detects known third-party cookies already present and highlights relevant categories
- **Multi-language** — auto-detects browser language, supports en/fr/de/es/it
- **Privacy policy link** — shown in the banner footer
- **`window.cookieConsent` API** — lets the site fire analytics/marketing scripts conditionally

## Goal
By end of today, `public/banner.js` is a compiled, minified vanilla JS file that handles the full GDPR consent lifecycle.
...
(async () => {
  ...
  // ─── Cookie scanner ──────────────────────────────────────────────────────────
  // Scans document.cookie for known third-party cookie patterns
  // and returns which categories are already present on the page.
  ...
  // Check stored consent — if versions match, skip the banner
  ...
  // Config changed (or first visit) — show the banner
  ...
  const c = config // alias for shorthand
  ...
  function render(showCustomize: boolean, analyticsChecked: boolean, marketingChecked: boolean) {
    ...
    wrapper.innerHTML = `
      <div style="${wrapStyle}">
        <div style="${boxStyle}">
          ${!showCustomize ? mainPanel() : customizePanel(analyticsChecked, marketingChecked)}
        </div>
      </div>
    `

    // Bind events
    document.getElementById("__cc_accept__")?.addEventListener("click", () =>
      respond("accepted", true, true)
    )
    document.getElementById("__cc_reject__")?.addEventListener("click", () =>
      respond("rejected", false, false)
    )
        ...
        <div style="display:flex;align-items:flex-start;justify-content:space-between;padding:12px 14px;border-bottom:1px solid ${c.primaryColor}11;">
          ...
          ${alwaysOn
            ...
            : `<label style="position:relative;display:inline-flex;align-items:center;cursor:pointer;flex-shrink:0;">
                ...
                <span id="__cc_${id}_track__" onclick="(function(el){var inp=document.getElementById('__cc_${id}_toggle__');inp.checked=!inp.checked;el.style.background=inp.checked?'${c.primaryColor}':'${c.primaryColor}33';var thumb=document.getElementById('__cc_${id}_thumb__');thumb.style.transform=inp.checked?'translateX(14px)':'translateX(0)';})(this.parentElement.querySelector('[id$=_track__]')||this)" style="display:block;width:36px;height:20px;border-radius:10px;background:${checked ? c.primaryColor : c.primaryColor + "33"};transition:background .2s;cursor:pointer;">
                  <span id="__cc_${id}_thumb__" style="display:block;width:16px;height:16px;border-radius:50%;background:white;margin:2px;transition:transform .2s;transform:${checked ? "translateX(14px)" : "translateX(0)"}"></span>
                </span>
              </label>`
          }
        </div>
  ...
  async function respond(
    ...
    try {
      ...
      // Fail silently — consent is stored locally regardless
    }
  }
...
### 3. Build script
`scripts/build-snippet.mjs`:
```javascript
import * as esbuild from "esbuild"
import { writeFileSync } from "fs"

const API_BASE = process.env.VITE_API_URL ?? "https://yourdomain.com"
...
pnpm build:snippet
```
Expected: `public/banner.js` around 4–6kb minified.
...
<head>
  ...
  <!-- Simulate a Google Analytics cookie to trigger scanner -->
  <script>document.cookie = "_ga=GA1.1.test; path=/"</script>
</head>
<body>
  <h1>Test page</h1>

  <script src="http://localhost:5173/banner.js" data-site-id="YOUR_SITE_ID" async></script>

  <script>
    window.addEventListener("cc:consent", (e) => {
      console.log("Consent:", e.detail)
      console.log("May fire analytics?", window.cookieConsent.hasAnalytics())
      console.log("May fire marketing?", window.cookieConsent.hasMarketing())
    })
  </script>
</body>
</html>
...
**First visit:**
- [ ] Banner appears with title, description, accept/reject buttons
- [ ] "Customize" button is visible when analytics or marketing categories are enabled
- [ ] Cookie scanner notice appears (set a `_ga` cookie manually to trigger it)
- [ ] Clicking "Customize" shows the category panel with toggles

**Categories:**
- [ ] Necessary row shows "Always active" (no toggle)
- [ ] Analytics/marketing toggles default to on
- [ ] Toggling off a category and saving sets `analytics: false` in localStorage
- [ ] "Save preferences" with analytics off, marketing off → `choice: "rejected"`
- [ ] "Save preferences" with analytics on, marketing off → `choice: "partial"`
- [ ] "Save preferences" with both on → `choice: "accepted"`

**Consent log:**
- [ ] ConsentLog row appears in Prisma Studio with correct `analytics`, `marketing`, `configVersion`

**Re-consent (invalidation):**
- [ ] Accept consent → banner disappears
- [ ] Go to dashboard → update banner title → Save
- [ ] Reload test page → banner reappears (configVersion changed)
- [ ] Accept again → stored consent has new version → banner won't show again

**Multi-language:**
- [ ] Set browser to French → "Personnaliser", "Enregistrer mes préférences" appear
- [ ] Override to `language: "de"` in dashboard → German even on English browser

**Global API:**
- [ ] `window.cookieConsent.hasAnalytics()` returns correct value after consent
- [ ] `window.cookieConsent.hasMarketing()` returns correct value
- [ ] `cc:consent` event fires with full detail object

**Persistence:**
- [ ] Reload page → banner does NOT appear (consent stored, version matches)
- [ ] Clear localStorage → banner appears again

---

## Definition of done
- [ ] `public/banner.js` built and under 8kb
- [ ] All three choice types work: accepted, rejected, partial
- [ ] Category toggles work in the customize panel
- [ ] Cookie scanner detects `_ga` as analytics, `_fbp` as marketing
- [ ] Stored consent contains `version`, `choice`, `necessary`, `analytics`, `marketing`
- [ ] Banner re-appears when `configVersion` changes
- [ ] Multi-language auto-detect works (fr, de, es, it, en)
- [ ] `window.cookieConsent.hasAnalytics()` and `.hasMarketing()` exposed
- [ ] HTML is properly escaped (XSS-safe)
- [ ] Privacy policy link shows when URL is set

---

## Common issues
- **Toggle not responding:** The inline `onclick` handler on the track span is needed because event delegation gets tricky inside `innerHTML`. If toggles feel unresponsive, check the `id` references match
- **"Customize" not showing:** Confirm `analyticsEnabled` or `marketingEnabled` is `true` in the config (check Prisma Studio)
- **Scanner always empty:** `document.cookie` only shows cookies for the current domain. To test, set a cookie in the browser console: `document.cookie = "_ga=test; path=/"`
- **configVersion not incrementing:** The PATCH endpoint uses `{ increment: 1 }` — if you see version staying at 1, check `day-03.md` step 1 and make sure you're calling the right endpoint
